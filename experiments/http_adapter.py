...
def main():
    ...
    # Only create HttpAdapter's table, not the full Base metadata
    ...
    # Verify
    with Session(engine) as session:
        adapters = session.query(HttpAdapter).all()
        print(f"Registered {len(adapters)} HTTP adapters:\n")
        print(f"  {'Name':<30} {'Method':>6} {'Rate':>6} {'Source':<20}")
        print(f"  {'-'*65}")
        for a in adapters:
            print(f"  {a.name:<30} {a.method:>6} {a.rate_limit_profile or 'none':>6} {a.source or '':<20}")
            print(f"    URL: {a.url_template[:70]}")
            if a.response_jmespath:
                print(f"    JMESPath: {a.response_jmespath[:60]}")
            print()
